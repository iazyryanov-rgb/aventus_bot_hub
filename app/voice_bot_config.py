...
System prompt в хабе хранится разрезанным на тематические блоки
(`main_prompt_blocks: dict[block_id, str]`). При Push в ElevenLabs блоки
склеиваются в один текст с anchor-комментариями (`# === BLOCK: <id> ===`),
при Pull — разрезаются обратно. Старый плоский `main_prompt: str`
поддержан как миграция (см. `_normalize_blocks`).

Config persisted to `data/voice_bot_config/<COMPANY_KEY>/<sector>.json`.
"""
...
# ---------------------------------------------------------------------------
# Block schema — system prompt разрезан на тематические блоки.
# ---------------------------------------------------------------------------
#
# `BLOCK_ORDER` — порядок склейки при Push (и порядок вкладок в UI).
# `BLOCK_TITLES`/`BLOCK_HELPS` — дефолтные подписи, i18n кладётся поверх.
# `block_vars_for(dyn_vars)` — построить per-block subset SIP-переменных
# для подсказки рядом с редактором.
...
BLOCK_TITLES: dict[str, str] = {
    ...
    "client_data":         "Client Data & Variables",
    "payment_logic":       "Payment Logic",
    "call_flow":           "Call Flow (Steps 1-7)",
    "special_scenarios":   "Special Scenarios",
    "confirmation_rules":  "Confirmation & Repetition",
    "compliance_scope":    "Compliance & Scope",
    "output_closing":      "Output & Closing Tools",
}

BLOCK_HELPS: dict[str, str] = {
    "identity_voice":      "Кто говорит, на каком языке, в каком тоне. Запрещённые слова, эскалация на человека.",
    "client_data":         "Технические заметки про SIP-headers и блок переменных клиента (имя, долг, DPD).",
    "payment_logic":       "Цель звонка, приоритет суммы (full → partial → 50.000) и approved consequences/benefits по DPD.",
    "call_flow":           "Сценарий звонка по шагам 1–7: identity → purpose → collect → objection → C&B → payment → close.",
    "special_scenarios":   "Третье лицо, уже заплатил, спорит долг, тишина.",
    "confirmation_rules":  "Как трактовать относительное время, в каком порядке подтверждать, как не переспрашивать.",
    "compliance_scope":    "Запреты комплаенса, рамки темы, naming для эскалации, приоритет system prompt над KB.",
    "output_closing":      "Формат речи (plain text), запрет квадратных скобок, короткое финальное закрытие, save+end.",
}


def block_vars_for(dynamic_variables: list[str]) -> dict[str, tuple[str, ...]]:
    """Build BLOCK_VARS map for a given per-company dynamic_variables list.
    Some blocks show the full list, some pick a subset by intent."""
    full = tuple(dynamic_variables)
    debt_subset = tuple(
        v for v in dynamic_variables
        if any(tok in v for tok in (
            "loan_debt", "partial_payment", "loan_dpd", "dpd",
            "discount", "extension", "ptp",
        ))
    )
    identity_subset = tuple(
        v for v in dynamic_variables
        if any(tok in v for tok in (
            "member_name", "user_first_name", "user_name", "agreement_signed",
        ))
    )
    return {
        "identity_voice":      (),
        "client_data":         full,
        "payment_logic":       debt_subset or full,
        "call_flow":           full,
        "special_scenarios":   identity_subset or full,
        "confirmation_rules":  (),
        "compliance_scope":    (),
        "output_closing":      (),
    }
...
# ---------------------------------------------------------------------------
# CO_ Credito365 — content of each block (адаптация коллекшен-промта 11labs
# на колумбийский рынок: pesos, español LATAM, Datacrédito / TransUnion,
# методы оплаты PSE / Nequi / Daviplata / Bancolombia / Efecty / Baloto).
# ---------------------------------------------------------------------------

_CO_IDENTITY_VOICE = """# Role and Context
...
_CO_CLIENT_DATA = """# INTEGRATION NOTES (for operator, not for spoken output)
...
_CO_PAYMENT_LOGIC = """# Primary Goal
...
_CO_SPECIAL_SCENARIOS = """# Third-Party Rule
...
_CO_CONFIRMATION_RULES = """# Time Understanding and Confirmation Rule
...
_CO_COMPLIANCE_SCOPE = """# Compliance Rules
Never threaten:
- arresto
- prisión
...
CO_VOICE_BLOCKS: dict[str, str] = {
    "identity_voice":     _CO_IDENTITY_VOICE,
    "client_data":        _CO_CLIENT_DATA,
    "payment_logic":      _CO_PAYMENT_LOGIC,
    "call_flow":          _CO_CALL_FLOW,
    "special_scenarios":  _CO_SPECIAL_SCENARIOS,
    "confirmation_rules": _CO_CONFIRMATION_RULES,
    "compliance_scope":   _CO_COMPLIANCE_SCOPE,
    "output_closing":     _CO_OUTPUT_CLOSING,
}
...
SEEDS: dict[str, dict[str, dict]] = {
    "CO_": {
        "collection": {
            ...
            "main_prompt_blocks": dict(CO_VOICE_BLOCKS),
    ...
    "PE_": {
        "collection": {
            ...
            "main_prompt_blocks": {bid: "" for bid in BLOCK_ORDER},
            "first_message": "",
...
ANCHOR_FMT = "# === BLOCK: {block_id} ==="
_ANCHOR_RE = re.compile(r"^#\s*===\s*BLOCK:\s*([a-z_][a-z0-9_]*)\s*===\s*$", re.MULTILINE)


def join_blocks(blocks: dict[str, str]) -> str:
    """Склейка блоков в плоский system_prompt для Push в ElevenLabs.
    Каждый непустой блок предварён anchor-комментарием, чтобы Pull
    мог разрезать обратно. Порядок — `BLOCK_ORDER`."""
    parts: list[str] = []
    for bid in BLOCK_ORDER:
        body = (blocks.get(bid) or "").rstrip()
        if not body:
            continue
        parts.append(f"{ANCHOR_FMT.format(block_id=bid)}\n{body}")
    return "\n\n".join(parts)


def split_blocks(text: str) -> tuple[dict[str, str], bool]:
    """Обратный разбор склеенного промта на блоки.
    Возвращает (blocks, recognized). Если якорей нет — кладёт весь
    текст в первый блок `BLOCK_ORDER[0]` и возвращает recognized=False,
    чтобы UI мог показать баннер «не распознана структура»."""
    out = {bid: "" for bid in BLOCK_ORDER}
    matches = list(_ANCHOR_RE.finditer(text or ""))
    if not matches:
        if text and text.strip():
            out[BLOCK_ORDER[0]] = text.strip()
            return out, False
        return out, True  # пустой текст = «всё распознано пустым»
    for i, m in enumerate(matches):
        ...
        if bid in out:
            out[bid] = chunk
        # Неизвестные block_id молча игнорируем — пользователь увидит, что
        # часть текста пропала, и сможет разложить вручную; альтернатива —
        # сваливать в "identity_voice" — путает сильнее.
    return out, True


# ---------------------------------------------------------------------------
# Heading-based auto-split — для одноразовой миграции существующих
# ElevenLabs-агентов, у которых в системном промте нет наших anchor'ов,
# но есть top-level `# Heading` секции в формате CO_ collection.
# ---------------------------------------------------------------------------
#
# Маппинг known section heading → block_id. Ключи нормализованы через
# `_norm_heading` (lower-case, схлопнутые пробелы). Покрывает два формата
# промтов, встречающихся в проде ElevenLabs у компаний хаба:
#
#  * EN markdown — `# Role and Context`, `# Tone`, `# Compliance Rules`, …
#    (см. SEED-промт CO_VOICE_BLOCKS в этом файле).
#  * ES UPPERCASE — `ROL Y CONTEXTO`, `TONO`, `PASO 1. VERIFICACIÓN DE
#    IDENTIDAD`, … (фактический prod-стиль CO1_collection_voice_bot_prod,
#    его подхватили AR_/PE_ при копировании). Без markdown-префикса
#    `#` — на отдельной строке только UPPERCASE-заголовок.
#
# `split_by_headings` использует карту как whitelist: строка считается
# заголовком только если её нормализованное значение есть здесь.
# Неизвестные строки остаются содержимым предыдущей секции — это
# защищает от ложных срабатываний на multi-line `#` комментариях
# и UPPERCASE-фразах внутри prose.
HEADING_TO_BLOCK: dict[str, str] = {
    # ---- EN markdown (`# Heading`) ----
    # identity_voice
    "role and context":                                "identity_voice",
    "language":                                        "identity_voice",
    "tone":                                            "identity_voice",
    "ai identity and human agent escalation rule":     "identity_voice",
    # client_data
    "integration notes (for operator, not for spoken output)": "client_data",
    "integration notes (operator-only, do not speak)": "client_data",
    "integration notes":                               "client_data",
    "client information":                              "client_data",
    # payment_logic
    "primary goal":                                    "payment_logic",
    "payment amount priority rule":                    "payment_logic",
    "approved consequences and benefits":              "payment_logic",
    # call_flow
    "call structure":                                  "call_flow",
    # special_scenarios
    "third-party rule":                                "special_scenarios",
    "third party rule":                                "special_scenarios",
    "already paid rule":                               "special_scenarios",
    "loan dispute vs not client rule":                 "special_scenarios",
    "silence rule":                                    "special_scenarios",
    # confirmation_rules
    "time understanding and confirmation rule":        "confirmation_rules",
    "confirmation order rule":                         "confirmation_rules",
    "already confirmed information rule":              "confirmation_rules",
    "repetition control rule":                         "confirmation_rules",
    # compliance_scope
    "compliance rules":                                "compliance_scope",
    "scope control":                                   "compliance_scope",
    "human agent and department naming rule":          "compliance_scope",
    "knowledge base usage rule":                       "compliance_scope",
    "knowledge base usage rule (structured)":          "compliance_scope",
    # output_closing
    "short final closing rule":                        "output_closing",
    "call ending tools":                               "output_closing",
    "no bracketed text in spoken output":              "output_closing",
    "output format":                                   "output_closing",

    # ---- ES UPPERCASE (prod-стиль CO1/PE/AR collection) ----
    # identity_voice
    "rol y contexto":                                  "identity_voice",
    "lengua obligatoria":                              "identity_voice",
    "tono":                                            "identity_voice",
    "palabras y expresiones prohibidas":               "identity_voice",
    "regla de identidad ai y escalamiento":            "identity_voice",
    # client_data
    "información del cliente desde los encabezados sip": "client_data",
    "informacion del cliente desde los encabezados sip": "client_data",
    "opción de extensión":                             "client_data",
    "opcion de extension":                             "client_data",
    "descuento activado":                              "client_data",
    "historial de promesas":                           "client_data",
    "pronunciación de fechas":                         "client_data",
    "pronunciacion de fechas":                         "client_data",
    "regla de valores vacíos":                         "client_data",
    "regla de valores vacios":                         "client_data",
    # payment_logic
    "estrategia según el tipo de préstamo":            "payment_logic",
    "estrategia segun el tipo de prestamo":            "payment_logic",
    "prioridad de propuestas de pago":                 "payment_logic",
    "pago mínimo — solo con dpd mayor o igual a 16":   "payment_logic",
    "pago minimo — solo con dpd mayor o igual a 16":   "payment_logic",
    "pago mínimo - solo con dpd mayor o igual a 16":   "payment_logic",
    "canales de pago":                                 "payment_logic",
    "plazo de pago":                                   "payment_logic",
    "regla de expiración del descuento":               "payment_logic",
    "regla de expiracion del descuento":               "payment_logic",
    "uso del historial de ptp":                        "payment_logic",
    "consecuencias y beneficios aprobados":            "payment_logic",
    # call_flow
    "estructura de la llamada":                        "call_flow",
    # special_scenarios
    "regla de terceros":                               "special_scenarios",
    "regla de disputa del préstamo":                   "special_scenarios",
    "regla de disputa del prestamo":                   "special_scenarios",
    "regla de silencio":                               "special_scenarios",
    # confirmation_rules
    "regla de información ya confirmada":              "confirmation_rules",
    "regla de informacion ya confirmada":              "confirmation_rules",
    # compliance_scope
    "nombre del departamento":                         "compliance_scope",
    "restricción de tema":                             "compliance_scope",
    "restriccion de tema":                             "compliance_scope",
    # output_closing
    "prohibición de placeholders en el habla":         "output_closing",
    "prohibicion de placeholders en el habla":         "output_closing",
    "cierre final corto":                              "output_closing",
    "herramientas":                                    "output_closing",
    "valores posibles de call_result":                 "output_closing",
    "formato de respuesta":                            "output_closing",
}
...
def _norm_heading(s: str) -> str:
    """Нормализация кандидата на заголовок для whitelist-lookup'а."""
    s = (s or "").strip().lstrip("#").strip()
    return _WS_RE.sub(" ", s).lower()


# Кандидат на заголовок секции. Два сценария:
#   1. EN markdown: строка `# Heading Text` (после `#` — буква, не `=`, не `-`).
#   2. ES UPPERCASE: строка БЕЗ markdown-префикса, состоящая
#      преимущественно из UPPERCASE-букв (с диакритикой), цифр и
#      разрешённой пунктуации. Минимум 5 символов.
# Whitelist-фильтр в `split_by_headings` отбрасывает кандидатов, которых
# нет в `HEADING_TO_BLOCK`, поэтому regex может быть избыточным.
_TOP_HEADING_RE = re.compile(
    r"^# [A-Za-z][^\n]*$"      # EN: `# Heading`
    r"|^[A-ZÁÉÍÓÚÑ0-9][A-ZÁÉÍÓÚÑa-z0-9 .,\-—«»()/_{}=]{4,}$",  # ES UPPERCASE
    re.MULTILINE,
)


def split_by_headings(text: str) -> tuple[dict[str, str], list[str]]:
    """Разбить текст на 8 блоков по known headings (`HEADING_TO_BLOCK`).

    Возвращает ``(blocks, unknown_headings)``:
      * ``blocks`` — словарь ``block_id → str`` (все 8 ключей всегда есть);
      * ``unknown_headings`` — заголовки-кандидаты, которых не оказалось
        в whitelist'е. Они НЕ начинают новую секцию (остаются
        содержимым предыдущей) — список нужен только для диагностики,
        чтобы оператор увидел, что промт содержит незнакомые секции
        и решил, нужно ли пополнить карту.

    Lead-in (текст перед первым known заголовком) уходит в первый
    блок ``identity_voice``.
    """
    buckets: dict[str, list[str]] = {bid: [] for bid in BLOCK_ORDER}
    unknown: list[str] = []

    if not text or not text.strip():
        return ({bid: "" for bid in BLOCK_ORDER}, unknown)

    candidates = list(_TOP_HEADING_RE.finditer(text))
    # Whitelist-фильтр: оставляем только кандидатов, чьё нормализованное
    # значение есть в HEADING_TO_BLOCK.
    matches: list[tuple[int, str]] = []  # (start, normalized_heading)
    for m in candidates:
        raw = m.group(0)
        norm = _norm_heading(raw)
        if norm in HEADING_TO_BLOCK:
            matches.append((m.start(), norm))
        else:
            unknown.append(raw.strip())

    if not matches:
        # Известных заголовков нет — всё в первый блок.
        return (
            {bid: (text.strip() if bid == BLOCK_ORDER[0] else "")
             for bid in BLOCK_ORDER},
            unknown,
        )

    # Lead-in перед первым known заголовком.
    lead = text[: matches[0][0]].strip()
    if lead:
        buckets[BLOCK_ORDER[0]].append(lead)

    for i, (start, norm) in enumerate(matches):
        nxt_start = matches[i + 1][0] if i + 1 < len(matches) else len(text)
        section = text[start:nxt_start].rstrip()
        bid = HEADING_TO_BLOCK[norm]
        buckets[bid].append(section)

    merged = {bid: "\n\n".join(buckets[bid]).strip() for bid in BLOCK_ORDER}
...
def _empty_blocks() -> dict[str, str]:
    return {bid: "" for bid in BLOCK_ORDER}
...
def _empty_config() -> dict:
    return {
        ...
        "main_prompt_blocks": _empty_blocks(),
        "first_message": "",
        "dynamic_variables": list(SIP_DYNAMIC_VARS),
    }


def _normalize_blocks(cfg: dict) -> dict:
    """Привести cfg к новой схеме `main_prompt_blocks`. Поддержка миграции
    из старого плоского `main_prompt: str` — текст без anchor-разделителей
    кладётся целиком в первый блок (`identity_voice`), оператор может
    переразложить вручную."""
    blocks = cfg.get("main_prompt_blocks")
    if isinstance(blocks, dict):
        merged = _empty_blocks()
        for bid in BLOCK_ORDER:
            v = blocks.get(bid)
            if isinstance(v, str):
                merged[bid] = v
        cfg["main_prompt_blocks"] = merged
        cfg.pop("main_prompt", None)
        return cfg

    legacy = cfg.get("main_prompt")
    if isinstance(legacy, str) and legacy.strip():
        parsed, _recognized = split_blocks(legacy)
        cfg["main_prompt_blocks"] = parsed
    else:
        cfg["main_prompt_blocks"] = _empty_blocks()
    cfg.pop("main_prompt", None)
    return cfg
    ...
    if p.exists():
        try:
            ...
            if isinstance(cfg, dict):
                base = _empty_config()
                base.update(cfg)
                return _normalize_blocks(base)
        except (OSError, json.JSONDecodeError):
            pass
    seed = (SEEDS.get(company_key) or {}).get(sector) or {}
    if seed:
        return _normalize_blocks(json.loads(json.dumps(seed)))  # deep copy
...
def get_main_prompt(
    ...
    """Полный system_prompt, склеенный из блоков (для Push / внешних потребителей)."""
    cfg = load_config(company_key, sector)
    return join_blocks(cfg.get("main_prompt_blocks") or {})
